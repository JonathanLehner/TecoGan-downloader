# TecoGAN single video
be sure to install tensorflow-gpu to use the GPU (otherwise it is very slow). The version cannot be too new since otherwise it breaks due to tensorflow.contrib being deprecated.
The speedup is almost not noticeable and GPU utilisation is very low <10%. It seems there is no way to make inference faster.
Update: Be sure to have CUDA 10 installed, which is independent from nvidia-smi
Update 2: No way to get my 1080Ti detected by tensorflow-gpu. I appreciate any suggestions. Pytorch works fine.
